...
MODEL = SVMDetector(path=settings.SVM_BASELINE_PATH, top_k=settings.COEF_MAX_TOP_K)


def on_click(text):
    scores, token_scores = MODEL.score(text, return_token_scores=True) 
    return round(scores, 4), token_scores
